...
def filter_motion_events(df_all):
    ...
    motions["motion"] = motions["from_url"].map(
        lambda x: x.split("&")[0].split("?motion=")[-1]
    )
...
def calculate_motion_stats(df) -> Dict[str, int]:
    ...
    motion_statistics = (
        df.groupby(by=["motion"], as_index=True)
        .agg(
            {
    ...
    motion_statistics = motion_statistics.rename(
        columns={
            "seconds_to_next_event_in_session": "Sidtid",
